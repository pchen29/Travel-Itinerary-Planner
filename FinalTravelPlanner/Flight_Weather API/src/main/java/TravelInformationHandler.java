...
public class TravelInformationHandler implements Handler {
    ...
    private void callApis(Parameters parameters) throws Exception {
        ...
        for (int location=0; location<allLocationAttractions.size(); location++){
                for (int category = 0; category < allLocationAttractions.get(location).size(); category++) {
                    for (int attraction = 0; attraction < allLocationAttractions.get(location).get(category).size(); attraction++) {
                        ...
                        days.get(attraction).add( " Day Time: " + category + "\n Attraction: " + attractionData.getName() +
                                "\n Ranking: "+ attractionData.getRating()  + "\n Web Url "+ attractionData.getWebsite()+"\n TripAdvisor Url: "+attractionData.getWeb_url());
                        }else
        ...
        for (List<HotelObjects.Data> hotels : allLocationHotels){
            if (hotels.size()!=0){
                allhotels.add("Hotel: " + hotels.get(0).getName() + "\n Class: " + hotels.get(0).getClass() + "\n Price: " + hotels.get(0).getPrice() + "\n Website: " + hotels.get(0).getWebsite() + "\n TriAdvisor Url: " + hotels.get(0).getTripAdvisor_web_url());
            }else
        ...
        //Convert Data into String Data Lists
        if (flight1.size()!=0){
            flight1quote= "Origin: "+parameters.getOrigin()+"\n Airport: "+parameters.getOriginStation()+"\n Destination: "+parameters.getDestination()+"\n Airport: "+ parameters.getDestinationStation()+"\n Time: "+flight1.get(0).getOutboundLeg().getDepartureDateTime()+"\n Min Price: "+flight1.get(0).getMinPrice();
        }else
            flight1quote="Flight Not Found For Specific Day";
        //Convert Data into String Data Lists
        if (flight2.size()!=0){
            flight2quote= "Origin: "+parameters.getDestination()+"\n Airport: "+parameters.getDestinationStation()+"\n Destination: "+parameters.getOrigin()+"\n Airport: "+ parameters.getOriginStation()+"\n Time: "+flight2.get(0).getOutboundLeg().getDepartureDateTime()+"\n Min Price: "+flight2.get(0).getMinPrice();
        }else
